            ...
            Some(new_id) => {
                ...
                match *old_id == new_id.child {
                    ...
                    false => { // global index are differents
                        ...
                        old_arena.removed_ids.insert(old_id.clone());
                        old_node.node_state = ArenaNodeState::IdChanged(old_id.clone(), new_id.child.clone());
                        old_node.id = new_id.child.clone();
                        if let Some(parent) = new_id.child.get_parent(&new_binding) {
                            old_arena.new_node_id_pairs.insert(Pairs::new(parent.clone(), new_id.child.clone()));
                        }
                        */
                    },
                    _ => {}
